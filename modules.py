...
class ContrastiveSWMsmall(nn.Module):
    def __init__(self, hinge=1., sigma=0.5, ignore_action=False, copy_action=False):
        super(ContrastiveSWMsmall, self).__init__()
        self.hinge = hinge
        self.sigma = sigma
        self.ignore_action = ignore_action
        self.copy_action = copy_action
    ...
    def energy(self, state, next_state, pred_next_state=None):
        """Energy function based on normalized squared L2 norm."""
    ...
    def contrastive_loss(self, state, next_state, predicted_next_state):
        ...
        self.pos_loss = self.energy(state, next_state, predicted_next_state)
        zeros = torch.zeros_like(self.pos_loss)

        self.pos_loss = self.pos_loss.mean()
        self.neg_loss = torch.max(
            zeros, self.hinge - self.energy(state, neg_state)).mean()
